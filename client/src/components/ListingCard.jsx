...
const ListingCard = ({
    ...
    const ph = listingPhotoPaths.map((item) =>
        `/api/${item.replace("public", "")}`
    );

    const [currentIndex, setCurrentIndex] = useState(0);

    const previ = () => setCurrentIndex((currentIndex - 1 + ph.length) % ph.length);
    const nexti = () => setCurrentIndex((currentIndex + 1) % ph.length);
        ...
        <div className="listing-card" onClick={() => navigate(`/properties/${listingId}`)}>
            <div className="slider-container">
                ...
                <div
                    ...
                    onClick={(e) => { e.stopPropagation(); previ(); }}
                >
                    <MdArrowBackIos />
                </div>
                <div
                    ...
                    onClick={(e) => { e.stopPropagation(); nexti(); }}
                >
                    <MdArrowForwardIos />
                </div>
            </div>
